...
class encoder(nn.Module):
    def __init__(self, input_channel, out_channel):
        ...
        self.conv2 = EncodeConvBlock(input_channel, int(out_channel/2))  # 32*32*128->16*16*256
        self.conv3 = EncodeConvBlock(int(out_channel / 2), out_channel)  # 16*16*256->8*8*512
    ...
    def __init__(self, fused=True):
        ...
        self.progression = nn.ModuleList(
            [
                ...
                StyledConvBlock(768, 256, 3, 1, upsample=True, fused=fused),  # 32
                StyledConvBlock(256, 256, 3, 1, upsample=True, fused=fused),  # 64
                StyledConvBlock(256, 128, 3, 1, upsample=True, fused=fused, norm=False),  # 128
            ]
        )
        self.to_rgb = EqualConv2d(128, 3, 1)
    def forward(self, stylem, x, m, v, mixing_range=(-1, -1), attack=False):
        if attack:
            out = m
        else:
        ...
        # out = torch.zeros(len(m), 512, 4, 4).cuda()
...
class StyledGenerator(nn.Module):
    ...
    def forward(
        ...
        return m, v, self.generator(self.style, x, m, v, mixing_range=mixing_range)
